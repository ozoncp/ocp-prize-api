...
var _ = Describe("Saver", func() {
	...
	Context("Test saving prizes with correct flush", func() {
		...
		BeforeEach(func() {
			...
			mockFlusher.EXPECT().Flush(gomock.Any(), gomock.Any()).Return(nil, nil).MinTimes(1)
		})
	...
	Context("Test saving prizes with error flush", func() {
		...
		BeforeEach(func() {
			...
			mockFlusher.EXPECT().Flush(gomock.Any(), gomock.Any()).Return(prizesToAdd, errors.New("error flushing")).MinTimes(1)
		})
